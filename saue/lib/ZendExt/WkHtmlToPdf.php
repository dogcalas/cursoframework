...
class ZendExt_WkHtmlToPdf{
     ...
     * @throws Exception
     */
    public function exportarPDF($html,$usuario,$fechaReporte,$reporte, $includeHeader, $includeFooter, $orientacion = "Landscape"){
        $opciones = array(
            'encoding' => 'UTF-8',
            'user-style-sheet' => 'css/reportes.css',
            'orientation' => $orientacion,
            'page-size' => 'A4'
        );
        if ($includeHeader){
            //$opciones['margin-top'] = '1cm';
            // $opciones['header-spacing'] = '1';
            //$opciones['header-html'] = $_SERVER['HTTP_HOST'].'/saue/reportes/headers/'.$reporte.'.html';
        }
        if ($includeFooter){
            $opciones['footer-left'] = 'Generado por '.$usuario;
            $opciones['footer-center'] = '[page] / [topage]';
            $opciones['footer-right'] = $fechaReporte;
            $opciones['footer-font-name'] = 'Calibri';
            $opciones['footer-font-size'] = '8';
            $opciones[] = 'footer-line';
        }
