...
/**
 * @see ZendExt_CodeGenerator_Php_Class
 */
require_once 'ZendExt/CodeGenerator/Php/Class.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class ZendExt_CodeGenerator_Php_File extends ZendExt_CodeGenerator_Php_Abstract
{

    /**
     * @var array Array of ZendExt_CodeGenerator_Php_File
     */
     ...
     * @var string
     */
    protected static $_markerDocblock = '/* ZendExt_CodeGenerator_Php_File-DocblockMarker */';
    protected static $_markerRequire = '/* ZendExt_CodeGenerator_Php_File-RequireMarker: {?} */';
    protected static $_markerClass = '/* ZendExt_CodeGenerator_Php_File-ClassMarker: {?} */';
    /**#@-*/
    ...
    public static function registerFileCodeGenerator(ZendExt_CodeGenerator_Php_File $fileCodeGenerator, $fileName = null)
    {
        if ($fileName == null) {
        ...
        if ($fileName == '') {
            ...
            throw new ZendExt_CodeGenerator_Php_Exception('FileName does not exist.');
        }
    ...
    public static function fromReflectedFileName($filePath, $usePreviousCodeGeneratorIfItExists = true, $includeIfNotAlreadyIncluded = true)
    {
        ...
        if ($realpath === false) {
            if ( ($realpath = Zend_Reflection_file::findRealpathInIncludePath($filePath)) === false) {
                require_once 'ZendExt/CodeGenerator/Php/Exception.php';
                throw new ZendExt_CodeGenerator_Php_Exception('No file for ' . $realpath . ' was found.');
            }
        }
    ...
    public static function fromReflection(Zend_Reflection_File $reflectionFile)
    {
        ...
        foreach ($reflectionFile->getClasses() as $class) {
            $file->setClass(ZendExt_CodeGenerator_Php_Class::fromReflection($class));
            ...
            for ($lineNum = 1; $lineNum <= count($bodyLines); $lineNum++) {
                if ($lineNum == $classStartLine) {
                    $bodyReturn[] = str_replace('?', $class->getName(), self::$_markerClass);  //'/* ZendExt_CodeGenerator_Php_File-ClassMarker: {' . $class->getName() . '} */';
        ...
        if (($reflectionFile->getDocComment() != '')) {
            ...
            $file->setDocblock(ZendExt_CodeGenerator_Php_Docblock::fromReflection($docblock));

            $bodyLines = explode("\n", $body);
            $bodyReturn = array();
            for ($lineNum = 1; $lineNum <= count($bodyLines); $lineNum++) {
                if ($lineNum == $docblock->getStartLine()) {
                    $bodyReturn[] = str_replace('?', $class->getName(), self::$_markerDocblock);  //'/* ZendExt_CodeGenerator_Php_File-ClassMarker: {' . $class->getName() . '} */';
     ...
     * setClass()
     *
     * @param ZendExt_CodeGenerator_Php_Class|array $class
     * @return ZendExt_CodeGenerator_Php_File
     */
    public function setClass($class)
    {
        if (is_array($class)) {
            $class = new ZendExt_CodeGenerator_Php_Class($class);
            $className = $class->getName();
        } elseif ($class instanceof ZendExt_CodeGenerator_Php_Class) {
            $className = $class->getName();
        } else {
            require_once 'ZendExt/CodeGenerator/Php/Exception.php';
            throw new ZendExt_CodeGenerator_Php_Exception('Expecting either an array or an instance of ZendExt_CodeGenerator_Php_Class');
        }
     ...
     * getClasses()
     *
     * @return array Array of ZendExt_CodeGenerator_Php_Class
     */
    ...
    public function generate()
    {
        ...
        $body = $this->getBody();
        if (preg_match('#/\* ZendExt_CodeGenerator_Php_File-(.*?)Marker:#', $body)) {
            $output .= $body;
            $body    = '';
        }
    ...
    public function write()
    {
        if ($this->_filename == '' || !is_writable(dirname($this->_filename))) {
            require_once 'ZendExt/CodeGenerator/Php/Exception.php';
            throw new ZendExt_CodeGenerator_Php_Exception('This code generator object is not writable.');
        }
