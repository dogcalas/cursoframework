...
class ezcWorkflowNodeInput extends ezcWorkflowNode {
    ...
    public function __construct($configuration = '') {
        ...
        foreach ($configuration as $key => $value) {
            ...
            if (is_int($key)) {
                ...
                /*
                 * esto no es bueno... tratar de implementar con un 
                 * switch considerando todos los casos, los mismos casos.
                 * 
                 * BasicType and value 'not condition' means
                 * an attribute and value
                 * ej
                 * idrol => 1000041
                 */
                if (is_string($key) && $this->isBasicType($value)) {
                    $variable = $key;
                    $condition = $value;
                } else {
                    if (!is_object($value) || !$value instanceof ezcWorkflowCondition) {
                        ...
                        $variable = $key;
                        if ($value instanceof ezcWorkflowCondition) {
                            $condition = $value;
                        }
                    }
                }
    ...
    private function isBasicType($value) {
        $type = gettype($value);
        $isBasicType = true;
        switch ($type) {
            case 'string':
