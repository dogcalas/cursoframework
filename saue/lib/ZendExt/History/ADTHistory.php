...
class ZendExt_History_ADTHistory {
	...
	function Datos($table,$limit,$start,$datostabla,$ncampos){
		if ($this->_db)
			return $this->_history_manager->Datos($table,$limit,$start,$datostabla,$ncampos);
		else
			throw new ZendExt_Exception ( 'E' );		
	}
	
	function camposTabla($table) {
		if ($this->_db)
			return $this->_history_manager->camposTabla($table);
		else
			throw new ZendExt_Exception ( 'E' );
	}
	function cantcamposTabla($table)
	{
		if ($this->_db)
			return $this->_history_manager->cantcamposTabla($table);
		else
			throw new ZendExt_Exception ( 'E' );
	}
