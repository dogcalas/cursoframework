...
ECONode = function (id, pid, dsc,icono, w, h, c, bc, target, meta){
	this.id = id;
	this.pid = pid;
	this.dsc = dsc;
	this.w = w;
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					{						
						s.push('<br>&nbsp;&nbsp;<img src="'+node.icono+'" align="absmiddle"  >&nbsp;');
						s.push(node.dsc);
					}
					s.push('</font>');
					s.push('</div>');		
...
ECOTree.prototype.add = function (id, pid, dsc,icono, w, h, c, bc, target, meta) {	
	...
	var node = new ECONode(id, pid, dsc,icono, nw, nh, color, border, tg, metadata);	//New node creation...
