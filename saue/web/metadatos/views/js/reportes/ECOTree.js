...
var altocanvas=100;
var anchocanvas=600;
var altocanvasInicial=100;
var anchocanvasInicial=600;
var idmenor;

var arrNodo = [];
var t = null, nodes,panelOrganigrama,paneltbar,nodoUltimoLevel,nodoprimero,nodoUltimo,leftPrimero;
ECONode = function (id, pid, dsc,icono,indice,nota,tipoPintar,pintado,agrupar,cantidadGrupo, w, h, c, bc, target, meta){
	this.id = id;
	this.pid = pid;
	this.dsc = dsc;
	this.w = w;
...
ECOTree = function (obj, elm) {
	this.config = {
		...
		nodeBorderColor : "#15428B",
		nodeSelColor : "#FFFFCC",
		levelColors : ["#5555FF","#8888FF","#AAAAFF","#CCCCFF"],
		levelBorderColors : ["#15428B","#6593CF","#AAAAFF","#DEECFD"],
		...
		expandedImage : '../../comun/recursos/iconos/organigrama/less.gif',
		collapsedImage : '../../comun/recursos/iconos/organigrama/plus.gif',
		transImage : '../../comun/recursos/iconos/organigrama/trans.gif'
	}
	...
	this.root = new ECONode(-1, null, null,null,null, 2, 2);
...
ECOTree.prototype._drawTree = function (nod,col) {
	var s = [];
	var node = null;
	var color = "";
	var border = "";
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					if(node.agrupar)
					 {
					   ECOTree._roundedRect(this.ctx,node.XPosition+10,node.YPosition+10,node.w,node.h,4);
					   ECOTree._roundedRect(this.ctx,node.XPosition+5,node.YPosition+5,node.w,node.h,4);
					 }
					ECOTree._roundedRect(this.ctx,node.XPosition,node.YPosition,node.w,node.h,5);
					
					
					}
					if(node.pintado==false )
					{
					if(nod && col==true && node.id==nod)
					{
					
					node.tipoPintar=true;					
					}
					
					if(nod && col==false && node.id==nod )
					{
					node.tipoPintar=false;
					...
					if (this.config.selectMode != ECOTree.SL_NONE)											
						s.push('onclick="javascript:ECOTree._canvasNodeClickHandler('+this.obj+',event.target.id,\''+node.id+'\');" onmouseover="javascript:PonerTollTip(\''+node.dsc+'\',\''+node.id+'\',\''+node.icono+'\',\''+node.nota+'\',\''+node.indice+'\')"');										
					s.push('>');					
					s.push('<div  style="text-align:center;float:right;width:20px;color:#7392C1;font-weight:bold">'+node.indice+'</div><font face=Verdana size=1>');					
					if (node.canCollapse) {
					...
					{	
						...
						s.push('<br>&nbsp;&nbsp;<img src="'+node.icono+'" align="absmiddle" width="16" height="16" >&nbsp;');
						s.push(node.dsc);
					}
					s.push('</font>');
					s.push('</div>');		
...
ECOTree.prototype.toString = function () {	
	...
	{
		case "CANVAS":
			s.push('<canvas id="ECOTreecanvas" width='+anchocanvas+' height='+altocanvas+'></canvas>');
			break;
...
ECOTree.prototype.UpdateTree = function (nod,col) {	
...
ECOTree.prototype.add = function (id, pid, dsc,icono,indice,nota,tipoPintar,pintado,agrupar,cantidadGrupo, w, h, c, bc, target, meta) {	
	...
	var node = new ECONode(id, pid, dsc,icono,indice,nota,tipoPintar,pintado,agrupar,cantidadGrupo, nw, nh, color, border, tg, metadata);	//New node creation...
...
ECOTree.prototype.Ajustar_XY_Despues_Colapsar =function (orientacion)
{
	...
	{  
	  ...
	  default:
	  ChangePosition(0);
	 /* var idmasizq=document.getElementById(idmenor)
      var temp=0;
	  var prim;
	  var largo=0;	
	  var xx;
	   ...
	   if(idmasizq)
        {  
            temp=posicionNumerica(idmenor,"left");
			largo=posicionNumerica(idmenor,"width");	
				  
	     }
	leftPrimero=posicionNumerica(nodoprimero,"left");
	if(temp<0)
	xx=50+largo+(-1*temp)+leftPrimero
	else
	xx=largo+leftPrimero-temp;
	this.config.topXAdjustment = xx;	
    this.config.topYAdjustment = -20;*/
	  break;
...
ECOTree.prototype.collapseNode = function (nodeid, upd) {
	...
	if (upd) this.UpdateTree(nodeid,this.nDatabaseNodes[dbindex].isCollapsed);
	this.Ajustar_XY_Despues_Colapsar(this.config.iRootOrientation);
}
