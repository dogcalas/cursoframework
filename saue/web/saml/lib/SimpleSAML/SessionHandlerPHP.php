...
class SimpleSAML_SessionHandlerPHP extends SimpleSAML_SessionHandler {
	 ...
	 */
	public function saveSession(SimpleSAML_Session $session,$namespace) {
		$_SESSION['SimpleSAML_Session'] = serialize($session);
	}
	 ...
	 */
	public function loadSession($sessionId = NULL,$namespace) {
		...
		if (!isset($_SESSION['SimpleSAML_Session'])) {
			return NULL;
		}

		$session = $_SESSION['SimpleSAML_Session'];
