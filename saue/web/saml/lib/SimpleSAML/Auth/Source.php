...
abstract class SimpleSAML_Auth_Source {
	...
	public static function getById($authId, $type = NULL) {
		...
		//
		// Esto es mio para la unificacion de un solo archivo de configuracion
		//$dir_file = __FILE__;
		//if('linux' == strtolower(PHP_OS))
		//$mydir = str_replace("lib\SimpleSAML\Auth\Source.php", "config\config.php", $dir_file);
		//echo "<pre>";print_r($mydir);die;
		$mydir = $_SERVER['DOCUMENT_ROOT'].'/saml/config/config.php';
		$myobjconfig = SimpleSAML_Configuration::loadFromFile($mydir,TRUE);
		$config = $myobjconfig->getMyConfigValue();
		if($authId == $config['ecotec.config']['appname']){
		
						$authConfig = array($config['ecotec.config']['authsource'],
											'entityID' => $config['ecotec.config']['idpmetadataonsamlspremote'],
											'idp' => $config['ecotec.config']['idpmetadataonsamlspremote'],
											'privatekey' => $config['ecotec.config']['secure']['privatekey'],
											'certificate' => $config['ecotec.config']['secure']['certificate']
											);
				}
				else{
				
						$authConfig = array($config['ecotec.config']['authtype'][0],
							'dsn' => $config['ecotec.config']['authtype']['connparameters']['gestor'].':host='.$config['ecotec.config']['authtype']['connparameters']['host'].';port='.$config['ecotec.config']['authtype']['connparameters']['port'].';dbname='.$config['ecotec.config']['authtype']['connparameters']['bd'],
							'username' => $config['ecotec.config']['authtype']['connparameters']['usuario'],
							'password' => $config['ecotec.config']['authtype']['connparameters']['password'],
							'query' => $config['ecotec.config']['authtype']['connparameters']['query']
							);
				}
