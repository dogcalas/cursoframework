...
window.webcam = {
	...
	swf_url: '../../views/js/reconocimiento/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	...
	get_html: function(width, height, server_width, server_height) {
		...
		if (this.ie) {
			...
			html += '<embed id="webcam_movie" wmode="Opaque" src="'+this.swf_url+'" loop="false" menu="false" quality="best" bgcolor="#000000" width="'+width+'" height="'+height+'" name="webcam_movie" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" flashvars="'+flashvars+'" />';
		}
	...
	get_movie: function() {
		...
		if (!this.loaded) return mostrarMensaje(3,perfil.etiquetas.lbErrorMovie);
	...
	snap: function(url, callback, stealth) {
		...
		if (typeof(stealth) != 'undefined.php') this.set_stealth( stealth );
		
		this.get_movie()._snap( 'sdsd', this.quality, this.shutter_sound ? 1 : 0, this.stealth ? 1 : 0 );
	},
	
	freeze: function() {
		// freeze webcam image (capture but do not upload)
	var a = this.get_movie()._snap('', this.quality, this.shutter_sound ? 1 : 0, 0 );
	...
	upload: function(url, callback) {
		...
		if (callback) this.set_hook('onComplete', callback);
		if (url) this.set_api_url(url);
		var a = this.get_movie();
		
		this.get_movie()._upload( this.api_url);
		
	userDetail.getForm().submit({
	     url:'reconocer',
	     ...
	     failure: function(form, action)
	     {
	       ...
	       mostrarMensaje(action.result.codMsg,action.result.mensaje);
	        ...
	        if(action.result.codMsg == 1) mostrarMensaje(action.result.codMsg,action.result.mensaje);
	     }
	});
	},
		...
		switch (type) {
			...
			case 'error':
				...
				if (!this.fire_hook('onError', msg)) {
					mostrarMensaje(3,perfil.etiquetas.lbErrorCamara);;
				}
				break;
